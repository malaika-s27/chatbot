...
export const useChatbot = ({ embedded = false } = {}) => {
  ...
  const handleSubClick = (id, label) => {
    console.log("handleSubClick called - removing submenu");
    setMessages((prev) => {
      const result = removeLastSubmenuMessage(prev);
      console.log("removeLastSubmenuMessage result:", result);
      return result;
    });
  ...
  const _makeResponsePrecise = (response) => {
